...
# Each scenario entry maps a modelling experiment to the directory holding its
# prediction tables. Only the models listed under 'models' are passed through
# to the financial layer; this prevents partially-tuned or exploratory runs
# from contaminating the reported financial metrics.
SCENARIOS = {
    "main_extended": {
...
def clean_percentage_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Convert any percentage-formatted strings to decimal fractions."""
    df = df.copy()
...
def mean_bias(y_true, y_pred) -> float:
    """Measure whether predictions are systematically high or low."""
    return float(np.mean(np.asarray(y_pred) - np.asarray(y_true)))


def corr_safe(y_true, y_pred) -> float:
    """Compute correlation only when both series have sufficient variation."""
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
    if len(y_true) < 2:
        return np.nan
...
def evaluate_series(y_true, y_pred) -> dict:
    """Return the financial performance metrics used in the dissertation."""
    return {
...
def load_prediction_tables(scenario_dir: Path) -> pd.DataFrame:
    """
    Load prediction tables produced by model_suite.py for one scenario.
    All three output files are included where they exist so that ensemble
    predictions and baseline results are handled consistently.
    """
    tables = []
...
def find_owned_capacity(df: pd.DataFrame) -> pd.Series:
    """
    Resolve owned capacity from whichever column combination is available.
    The panel may store owned_capacity_mw directly or require construction
    from installed capacity and ownership fraction depending on the data build.
    """
...
def risk_summary(s: pd.Series) -> dict:
    """
    Summarise the distributional properties of a revenue series.

    The P10 and P05 quantiles are used as downside risk measures throughout
    the dissertation, consistent with industry practice in renewable energy
    yield analysis. Coefficient of variation normalises volatility by the
    mean, allowing comparison across revenue levels.
    """
    s = s.dropna()
...
def prepare_panel() -> pd.DataFrame:
    """
    Load the cleaned modelling panel and retain the columns needed for
...
def standardise_actual_generation_column(merged: pd.DataFrame, scenario_name: str) -> pd.DataFrame:
    """
    Resolve the actual generation column after merging prediction and panel data.
    """
    merged = merged.copy()
...
def add_revenue_columns_for_fixed_share(
    ...
    """
    Construct merchant, fixed-price, and blended revenue measures for one
    revenue-share assumption.
    """
    df = df.copy()

    # The fixed-price reference is a 12-month trailing mean of the observed
    # wholesale price. This is used as a transparent proxy for long-term PPA
    # pricing because asset-level contract data is not publicly available.
    # An expanding mean is used as a fallback in the early window where fewer
    ...
    # Merchant revenue uses the contemporaneous spot price, capturing full
    ...
    # Fixed-price revenue uses the trailing-average reference price as a proxy
    # for contracted revenue streams such as PPAs or Contracts for Difference.
    ...
    # Blended revenue combines fixed and merchant components in the ratio
    # specified by fixed_share, representing a portfolio with partial
    # contracted revenue and partial spot-market exposure.
...
def run_one_scenario(
    ...
    """
    Apply the full financial translation for one modelling scenario: merge
    predictions with asset capacity and price data, construct revenue under
    all fixed-share assumptions, and compute asset-level and portfolio-level
    accuracy and risk metrics. The downside RMSE is computed over the subset
    of months where realised revenue falls below the P10 threshold.
    """
    ...
    # The forecasting model predicts generation per MW to ensure comparability
    # across assets. Multiplying by owned capacity re-introduces asset scale so
    # that downstream revenue calculations reflect the fund's economic exposure.
    ...
    # Revenue is computed separately under each fixed-share assumption so that
    # sensitivity to contract structure can be evaluated without re-running
    ...
    # Asset-level metrics are retained alongside portfolio metrics to isolate
    # whether forecast errors originate at the individual asset level or
    # emerge from aggregation and price covariance effects.
    asset_rows = []
    ...
    # Portfolio revenue is the sum of asset revenues within each calendar month.
    # Model rankings at the portfolio level can differ from asset-level rankings
    # because correlated errors amplify under summation and the merit-order
    # effect introduces non-linearity between generation and revenue.
    portfolio_monthly = (
...
def main():
    """
    Run the financial translation layer for every configured scenario and
    write the consolidated output tables. The final summary merges portfolio
    accuracy metrics with predicted distributional statistics to produce the
    model comparison table reported in the dissertation results chapter.
    """
