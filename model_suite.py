...
# Convergence warnings are suppressed during grid search to keep console output
# readable; they do not affect model selection or final fitted parameters.
...
# Scale-related variables are excluded from the feature set because the
# forecasting target, generation_per_mw, is already normalised by installed
# capacity. Including raw capacity would allow the model to learn asset size
# rather than climate-driven generation dynamics.
...
def rmse(y_true, y_pred) -> float:
    """Compute root mean squared error for a prediction vector."""
    return float(np.sqrt(mean_squared_error(y_true, y_pred)))


def mape(y_true, y_pred, eps: float = 1e-8) -> float:
    """Compute mean absolute percentage error with a stability constant."""
...
def mean_bias(y_true, y_pred) -> float:
    """Measure directional bias in predictions."""
    return float(np.mean(np.asarray(y_pred) - np.asarray(y_true)))


def directional_correlation(y_true, y_pred) -> float:
    """Return Pearson correlation when both series vary sufficiently."""
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
    if len(y_true) < 2:
        return np.nan
...
def evaluate_predictions(y_true, y_pred) -> dict:
    """
    Return the evaluation metrics used in the dissertation results tables.
    Multiple metrics are reported because RMSE is sensitive to outliers while
    MAE provides a more stable central-tendency measure; both are retained
    to allow consistent comparison with prior literature.
    """
    return {
...
def clean_percentage_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert percentage-formatted strings to decimal fractions across the dataset.
    """
    df = df.copy()
...
def add_calendar_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add calendar features that encode seasonal cycles and long-run trend.

    Month is encoded as sine and cosine projections rather than a raw integer
    to preserve the circular continuity between December and January, which
    a linear encoding would break.
    """
    df = df.copy()
...
def add_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Construct lagged, rolling, interaction, and non-linear features.

    All lags and rolling windows are computed within asset groups and shifted
    by at least one period before the rolling window is applied, ensuring
    no future information leaks into the training features. Interaction terms
    are constructed as element-wise products of climate variables with
    technology dummies, allowing the model to learn technology-specific
...
def split_by_dates(df: pd.DataFrame, train_end: str, valid_end: str):
    """Create chronological train, validation, and test subsets."""
...
def baseline_asset_mean(train_df: pd.DataFrame, target_df: pd.DataFrame) -> pd.Series:
    """Predict each asset using its historical mean generation."""
...
def baseline_asset_month_seasonal(train_df: pd.DataFrame, target_df: pd.DataFrame) -> pd.Series:
    """Predict each asset-month using historical seasonal averages."""
...
def baseline_last_value(train_df: pd.DataFrame, target_df: pd.DataFrame) -> pd.Series:
    """Predict each asset using the most recent observed value."""
...
def get_feature_sets(df: pd.DataFrame) -> dict[str, dict[str, list[str]]]:
    """
    Define progressively richer feature sets for robustness analysis.

    Three nested feature sets support the robustness analysis in the results
    chapter. Core uses only contemporaneous climate variables and first-order
    lags. Extended adds longer lags, rolling statistics, and interaction terms.
    Autoregressive further adds lagged and rolling generation variables, which
    capture temporal persistence in the target series.
    """
    exclude_base = {
...
@dataclass
class ModelSpec:
    """Container for model construction, tuning grid, and scaling requirements."""
    name: str
...
def maybe_get_xgboost_spec() -> list[ModelSpec]:
    """Optionally include XGBoost if the package is available."""
    if not RUN_XGBOOST:
        return []
    try:
...
def get_model_specs() -> list[ModelSpec]:
    """Return the full set of candidate models evaluated in the dissertation."""
    specs = [
...
def make_preprocessor(numeric_features: list[str], categorical_features: list[str], scaled_numeric: bool):
    """
    Build the preprocessing pipeline used before each estimator.

    Median imputation is used rather than mean imputation because climate
    distributions are skewed in some months. Scaling is applied only for
    linear models and MLP, which are sensitive to feature magnitude; tree-based
    models receive unscaled inputs.
    """
...
def fit_pipeline(estimator, preprocessor, X_train, y_train) -> Pipeline:
    """Fit a preprocessing-plus-estimator pipeline on the training sample."""
...
def time_series_cv_score(df_train_valid: pd.DataFrame, numeric_features, categorical_features, model_spec: ModelSpec, params: dict, n_splits: int = 4):
    """
    Estimate time-series cross-validation performance for one parameter setting.
    """
...
def select_best_params(train_df: pd.DataFrame, valid_df: pd.DataFrame, numeric_features, categorical_features, model_spec: ModelSpec):
    """
    Search the predefined parameter grid and retain the best validation model.
    """
    rows = []
...
def refit_and_predict(train_valid_df: pd.DataFrame, test_df: pd.DataFrame, numeric_features, categorical_features, model_spec: ModelSpec, params: dict):
    """Refit the selected model on train+validation data and score on test data."""
    estimator = model_spec.estimator_factory()
    estimator.set_params(**params)
    preprocessor = make_preprocessor(numeric_features, categorical_features, model_spec.scaled_numeric)
...
def try_export_importance(pipe: Pipeline) -> pd.DataFrame | None:
    """
    Export model importance information when the fitted estimator supports it.
    """
    try:
...
def run_scenario(df: pd.DataFrame, scenario_name: str, split_config: dict, feature_spec_name: str, subset_label: str, subset_filter: Callable[[pd.DataFrame], pd.DataFrame] | None):
    """
    Execute one full modelling experiment: split the data, fit and tune all
    model specifications, evaluate on the test set, and write all outputs.
    A run_metadata.json file is saved alongside results to record the exact
    feature counts and split boundaries used, supporting reproducibility.
    """
    ...
    # The three baseline methods represent the range of current industry
    # practice, from simple asset-level averages to the asset-month seasonal
    # model that most closely approximates EYA-style forecasting. Baselines
    # are fitted on the training split only and are not tuned.
    ...
    # Models are selected on the validation period to avoid test-set leakage.
    # The winning configuration is then refit on train+validation combined
    # before evaluation on the held-out test set, maximising the effective
    # training sample for the final model.
    ...
    # A simple equal-weight average of the three best validation models is
    # used as a model-agnostic ensemble. This tests whether combining
    # predictions from complementary model classes reduces forecast variance
    # without requiring a second-stage stacking estimator.
...
def main():
    """
    Run every scenario defined in the scenarios list. Each scenario is a
    combination of data split, feature set, and asset subset. Results from
    all scenarios are consolidated into master ranking tables that the
    financial translation layer reads downstream.
    """
